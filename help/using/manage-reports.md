...
AEM Assets Essentials 環境では、レポートダッシュボードを通じて包括的なレポート機能が提供されます。 この機能により、ユーザーは、指定された期間（1 回限りから、日別、週別、月別、年別の間隔まで）内のアセットのアップロードとダウンロードの詳細を示す CSV レポートを生成してダウンロードできます。
...
既存のレポートは、「**実行されたレポート**」タブの下に表示されます。 「**レポート**」をクリックし、「**実行されたレポート**」を選択すると、ステータスが&#x200B;**完了**&#x200B;となり、ダウンロードの準備が整ったことを示す、作成済みのすべてのレポートが表示されます。 レポートをCSV形式でダウンロードするか、レポートを削除するには、レポート行を選択し、**CSVをダウンロード**&#x200B;または&#x200B;**削除**&#x200B;を選択します。
...
1. レポートダイアログボックスには、以下の情報が表示されます。
   ...
   1. **「レポートをスケジュール」を切り替え：**&#x200B;レポートを後で実行するようにスケジュールするか、繰り返し実行するように切り替えます。
      ![レポートのスケジュール](/help/using/assets/schedule-reports1.svg)
...
キャンセルしたスケジュールを再開するには、レポート行を選択し、「**スケジュールを再開**」をクリックします。 再開すると、次のランタイムエントリが再び表示され、ステータスが「進行中」と表示されます。
![スケジュールの再開](/help/using/assets/resume-schedule.png)
