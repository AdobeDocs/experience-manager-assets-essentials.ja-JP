...
>abstract="使いやすい WYSIWYG インターフェイスを使用すると、画像やテキストのバナーをその場で作成およびパーソナライズし、Dynamic Media URL をファーストパーティまたはサードパーティのアプリケーションに埋め込むことで、非常に魅力的なエクスペリエンスを実現できます。 お試しください。"
...
1. Assets Essentialsに移動し、左側のパネルにある&#x200B;**[!UICONTROL Dynamic Media Assets]**&#x200B;をクリックします。
...
**[!UICONTROL スマートテキストのサイズ変更]**：フォントサイズと長さをスマートに調整して、指定された領域にテキストを最適に収めるには、**[!UICONTROL スマートテキストのサイズ変更]**（[コピーフィット](https://experienceleague.adobe.com/ja/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/http-protocol-reference/text-formatting/r-copy-fitting)）を必ず含めてください。 この機能は、テキストのオーバーフローを防ぎ、テキストの下部の余分なスペースを最小限に抑えます。
...
**X:**&#x200B;含める場合は、URLのパラメーターの値を変更して、レイヤーを中心線に沿って水平方向に移動し、テンプレート平面のX軸と平行にします。
**Y:** URLのパラメーターの値を変更して、レイヤーを中心線に沿って垂直方向に移動します。レイヤーをテンプレーンのY軸に平行に移動します。
**幅：** URL内のパラメーターの値を変更して、レイヤーの幅を調整するためにを含めます。
**高さ：** URL内のパラメーターの値を変更して、レイヤーの高さを調整するためにを含めます。
**非表示：** 0 （表示）と1 （非表示）を使用して、テンプレート内のレイヤーを非表示または表示します。
**Source:** URL内のパラメーターの値の画像パスを変更して、レイヤーの画像を新しい画像に置き換える場合に含めます。

**テキストの書式設定パラメーター：**

URL のパラメーター値を更新して、URL からテキスト、フォント、カラー、サイズを編集するには、以下のパラメーターを含めます。

**テキスト：** URLからテキストを更新する場合に含めます。
**フォントファミリー：** URLからテキストのフォントを更新する場合に含めます。
**フォントサイズ：** URLからテキストのフォントサイズを更新する場合に含めます。
**テキストの色：** URLからテキストのフォントカラーを更新する場合に含めます。
...
1. キャンバスページで、「**[!UICONTROL プレビュー]**」をクリックします。 **[!UICONTROL Assets Essentials]** **>** **[!UICONTROL Dynamic Media Assets]** **>**&#x200B;に移動して、テンプレートを検索して選択することもできます&#x200B;**>** クリック **[!UICONTROL テンプレートを編集]** **>** クリック **[!UICONTROL プレビュー]**。 プレビューページには、テンプレート、そのパラメーター（パラメーター化されたレイヤとプロパティ）、公開ステータスおよび「**[!UICONTROL 公開]**」オプションが表示されます。
...
1. 「**[!UICONTROL URL をコピー]**」をクリックします。 **[!UICONTROL URL をコピー]**&#x200B;ダイアログボックスが表示されます。 表示されている URL を選択してコピーします。 URLの最初のパラメーターが疑問符&#x200B;**（?）**&#x200B;の後から始まることに注意してください キーと値のペアは&#x200B;**$**&#x200B;で始まり、**&amp;**&#x200B;で終わります。 キーと値は等号&#x200B;**（=）**&#x200B;で区切られ、キーが左側、値が右側になります。
...
1. Assets Essentialsで、**[!UICONTROL Dynamic Media Assets]**&#x200B;をクリックします。
2. テンプレートの場所に移動します。
3. テンプレートを選択します。
