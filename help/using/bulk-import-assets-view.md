...
1. 「**[!UICONTROL ソースフォルダー]**」フィールドでデータソースのアセットを含むルートフォルダーの名前を指定します。
   ...
   >* Dropbox アプリケーションの作成時に **アプリフォルダー** を選択した場合、アセットが含まれるフォルダーが `https://www.dropbox.com/home/Apps/BulkImportAppFolderScope/bulkimport-assets` に存在するときは、「**[!UICONTROL Source フォルダー]**」フィールドで `bulkimport-assets` を指定します。`BulkImportAppFolderScope` はアプリケーションの名前を表します。`Apps` この場合、`home` の後に自動的に追加されます。
...
アセットやフォルダーを一括で読み込む [!DNL Experience Manager Assets]、読み込みソースに存在する情報の構造全体が読み込まれます。[!DNL Experience Manager] は、アセット名とフォルダー名に含まれる特殊文字に対する組み込みルールに従うので、これらのファイル名のサニタイズが必要になります。 フォルダー名とアセット名の両方について、ユーザーが定義したタイトルは変更されず、`jcr:title` に保存されます。
...
また、ドロップダウンオプションから **[!UICONTROL 保存済みの読み込み]** および **[!UICONTROL スケジュール済みの読み込み]** を表示することもできます。
