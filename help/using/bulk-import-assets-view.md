...
# Assets Essentials を使用したアセットの一括読み込み   {#bulk-import-essentials}
...
>additional-url="https://images-tv.adobe.com/mpcv3/4477/98bce651-721c-442e-98b8-c43e7708e44c_1698834577.854x480at800_h264.mp4" text="ビデオを観る"
...
| AWS | <ul> <li>AWS リージョン </li> <li> AWS バケット <li> AWS アクセスキー </li><li> AWS アクセスシークレット </li></ul> |
...
1. [Dropbox アカウント ](https://www.dropbox.com/developers)にログインし、**[!UICONTROL アプリの作成]**&#x200B;をクリックします。<br>Enterprise Dropbox アカウントを使用している場合は、Content Admin ロールにアクセスできる必要があります。
...
1. 「**[!UICONTROL ソースフォルダー]**」フィールドでデータソースのアセットを含むルートフォルダーの名前を指定します。
   ...
   >* Dropbox アプリケーションの作成中に&#x200B;**App フォルダー**&#x200B;を選択し、アセットを含むフォルダーが`https://www.dropbox.com/home/Apps/BulkImportAppFolderScope/bulkimport-assets`に存在する場合は、**[!UICONTROL Source フォルダー]** フィールドに`bulkimport-assets`を指定します（`BulkImportAppFolderScope`はアプリケーション名を表します）。`Apps` この場合は、`home`の後に自動的に追加されます。

1. （オプション）ファイルが [!DNL Experience Manager Assets] に読み込まれた後、ソースデータストアから元のファイルを削除するには、「**[!UICONTROL 読み込み後にソースファイルを削除]**」オプションを選択します。
...
アセットまたはフォルダーを一括インポートすると、[!DNL Experience Manager Assets]はインポートソースに存在するものの構造全体をインポートします。[!DNL Experience Manager] アセット名とフォルダー名の特殊文字に対する組み込みのルールに従うので、これらのファイル名はサニタイズする必要があります。 フォルダー名とアセット名の両方について、ユーザーが定義したタイトルは変更されず、`jcr:title` に保存されます。
...
ドロップダウンオプションから、**[!UICONTROL 保存済みインポート]**&#x200B;と&#x200B;**[!UICONTROL スケジュール済みインポート]**&#x200B;を表示することもできます。
...
## ビデオ：Assets Essentials を使用したアセットの一括読み込み
