...
Assets Essentials の最新リリースは 2023年3月30日（PT）にリリースされました。

最近追加された機能のリストは次のとおりです。

**アセットメタデータの一括アップデート**

メタデータの一括アップデートを使用すると、複数のアセットに対して、共通のメタデータアップデートを同時に実行できます。 レコードを個別に更新する必要はなく、検索、フォルダー、コレクションを通じてアクセスするアセットにプロパティをすばやく適用することができます。
