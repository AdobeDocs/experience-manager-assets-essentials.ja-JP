...
Assets Essentialsの現在のリリースは 2023 年 3 月 30 日にリリースされました。

最近追加された機能のリストは次のとおりです。

**アセットメタデータの一括更新**

メタデータの一括更新を使用すると、複数のアセットに対して、共通のメタデータ更新を同時に実行できます。 レコードを個別に更新する必要はなく、検索、フォルダー、コレクションを通じてアクセスするアセットにプロパティをすばやく適用することができます。
