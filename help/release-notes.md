...
Assets Essentialsの現在のリリースは 2023 年 5 月 24 日にリリースされました。

最近追加された機能のリストは次のとおりです。

**最もダウンロードされたアセット**

My Workspace に、 [!UICONTROL コンテンツ] 」セクションに入力します。 また、リストに表示されている各アセットの形式タイプとダウンロード数も表示できます。
...
My Workspace には、Insights、タスクおよびコンテンツのウィジェットが含まれます。 環境設定に基づいて、ワークスペースでのこれらのウィジェットの表示方法を設定できます。
